    ...
    stats?.let {
        MKExpandableSection(title = stringResource(R.string.records_series)) {
            Column(Modifier.fillMaxWidth().padding(bottom = 8.dp)) {
                ...
                if (it.top6Count > 0)
                    MKStatRow(label = stringResource(R.string.top6_count), value = it.top6Count.toString())
                if (it.bot6Count > 0)
                    MKStatRow(label = stringResource(R.string.bot6_count), value = it.bot6Count.toString())
            }
        }
    }
}
