...
- [ ] 🟡 **B5 — `WarTrack.diffScore` force le mode 12p.** La constante `82` est centralisée (G2 ✅). Mais `WarTrack.diffScore` ([WarTrack.kt:29](../app/src/main/java/fr/harmoniamk/statsmkworld/model/firebase/WarTrack.kt)) appelle encore `positionToPoints(false)` en dur. `WarDetails` lui passe correctement `is24p`. Risque réduit depuis B2 ✅, mais `WarTrack` reste aveugle au mode.
