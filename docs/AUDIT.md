...
- [x] ✅ **B5 — `WarTrack.diffScore` force le mode 12p.** Appliqué : `diffScore` est désormais une fonction `diffScore(is24p: Boolean = false)` ([WarTrack.kt:27](../app/src/main/java/fr/harmoniamk/statsmkworld/model/firebase/WarTrack.kt)) utilisant `positionToPoints(is24p)`. Les 3 appelants propagent le mode : `LineChartExtension` (via `WarTrackDetails.is24p`) et `withTrackStats` (via le `is24p` local). Le complément adverse reste à `82` (cohérent avec `WarTrackDetails`).
...
- [x] ✅ **B7 — Erreurs réseau silencieuses.** Appliqué (volet journalisation) : les datasources MKCentral/Discord loggent désormais via Crashlytics (`recordException(t)` en `onFailure`, `log(...)` sur corps d'erreur HTTP). Les signatures nullables sont **conservées volontairement** : propager `NetworkResponse.Error` partout cascaderait sur ~10 appelants (`FetchUseCase`, VMs) — refonte à planifier séparément.
...
- [x] ✅ **G2 — Constantes de scoring magiques.** Partiellement appliqué : `ScoringConstants.kt` regroupe `MAX_POINTS_PER_TRACK_12P` (82), `MID_WAR_SCORE` (492), `MID_TRACK_SCORE` (41), `TOTAL_24P_SCORE` (1728), `DEBUG_PLAYER_ID`. Reste : rôles `0/1/2` (→ enum `Role`) et équipe `"123456789"` (→ constante/`BuildConfig`).
