...
- [ ] 🟡 **D20 — `WarScoreView` monolithique (~456 lignes)** avec imbrication profonde `when/Column/Row/forEach`. → Découper (`WarScore24PDisplay`, `WarScore1v1Display`, `WarPenaltiesSection`, `TeamLogoImage`). *(Partiel : les deux branches `when(is24p)` sont extraites en composables privés `WarScore24pView`/`WarScore12pView`, et les tris/sommes/pénalités sont mémoïsés — voir passe fluidité Compose ci-dessous. Restent `WarPenaltiesSection` et `TeamLogoImage` (⊂D16).)*
