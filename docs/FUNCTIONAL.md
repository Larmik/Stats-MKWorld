...
**Wizard 4 étapes sur un seul écran** (rendu **pixel-perfect** vs la maquette du prototype UX — écran `addtrack`, rules 13/15). En tête, le **stepper cliquable** partagé (`MKStepper`) `Circuit` → `Intermission` → `Positions` → `Résumé`. Les étapes basculent **dynamiquement** (état `step` du `AddTrackViewModel`, **aucune re-navigation** ni transition slide, rule 11) ; l'étape courante est pilotée par le VM. Le **gating** du stepper : `Intermission`/`Positions` accessibles une fois le circuit choisi, `Résumé` une fois toutes les positions saisies. Écran du graphe racine poussé **par-dessus** CurrentWar → **sans bottombar** (rule 17).

1. **Circuit** : recherche par nom/label + grille des 30 circuits. Choisir un circuit **réinitialise la saisie de positions** et **avance à l'Intermission**.
2. **Intermission** (12p comme 24p) : chip « **Aucune** » (actif par défaut) ou grille des circuits alternatifs (`Maps.intermissionsTo(...)`) enchaînés ; resélectionner le même circuit l'annule. Boutons « Précédent » / « Suivant · Positions ».
3. **Positions** : saisie **joueur par joueur** — carte de progression `Joueur n / total` + barre, rappel du joueur courant, grille de positions cliquables (1–12 ou 1–24, cellules plus petites en 24p) ; les positions déjà prises sont verrouillées. La **dernière** position bascule **automatiquement** au Résumé. Bouton « Précédent » (retour Intermission, réinitialise la saisie).
4. **Résumé** : carte en-tête (circuit + **score de la manche calculé** — barème `positionToPoints`, total 82 en 12j). En 12p : `score - adverse` et `(±diff)` ; en 24p : progression `score actuel → nouveau score` (l'adversaire est saisi ailleurs). Grille **Positions & shocks** : une carte par joueur (`PlayerCell`) avec **compteur de shocks − / +**. « Confirmer » écrit le `WarTrack` (indices du/des circuit(s), positions, shocks) dans Firebase et met à jour les scores.

- **Retour en arrière (rule 11 wizard)** : revenir au **Circuit** = remise à zéro complète (circuit, intermission, positions, shocks, score) ; revenir aux **Positions** vide la line-up en cours. Le bouton retour système recule d'une étape (et applique la réinitialisation de l'étape rejointe), puis quitte depuis l'étape Circuit.
