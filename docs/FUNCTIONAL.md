...
- **Positions/Shocks** : **une ligne par joueur** (cellules `PlayerShockCell` partagées avec le Résumé de l'ajout de course), pré-remplie avec la position et les shocks actuels. **L'ordre des lignes reste stable** (ordre des joueurs par nom) pendant l'édition — les cellules ne sautent pas de place quand on change une position. Chaque ligne est une **petite grille alignée** : colonne des `−` | colonne centrale (**position** en haut, **sans encadré blanc** ; **icône shock collée à gauche du compteur** en bas) | colonne des `+`, le tout **centré**. Deux contrôles ± : la **position** (bornée **1..12** en 12p / **1..24** en 24p, − / + désactivés aux extrémités) se met à jour **en direct**, et les **shocks** (hors calcul du score).
