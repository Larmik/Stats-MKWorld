...
- **Carte « Scores des joueurs »** : **une ligne compacte par joueur** — nom à gauche (+ nb de courses jouées si le joueur n'a pas tout joué, + pastille de shock si applicable), « **N pts** » à droite (`Arrangement.SpaceBetween`), pour gagner de la place verticale.
...
- **Grille des courses jouées** (`.trackgrid`, eyebrow « Courses jouées · N ») : cellules compactes — **illustration du circuit** (rectangle à coins arrondis) à **gauche**, à **droite** le **score total « hôte-adverse »** (ex. « 44-38 », `WarTrackDetails.displayedResult`) et, **en dessous**, la **différence de score colorisée** (vert si > 0, rouge si < 0, blanc si = 0). Clic → détail de la course. Grille 2 colonnes en lignes chunkées (pas de `LazyVerticalGrid` imbriqué dans le `LazyColumn`).
