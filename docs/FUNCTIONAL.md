...
- **Démarrer une nouvelle saison** (onglet **Équipe** du profil, **leader strict** uniquement — `role == 2`, pas les admins) : au clic, une **confirmation** insiste sur le caractère **important et irréversible** de l'action. À la validation, la saison en cours est **close** (sa date de fin = l'instant du clic) et une **nouvelle saison** démarre (numéro suivant, sans date de fin). Sa date de début respecte la **règle des bornes** : elle commence **le lendemain** de la clôture (date de fin + 1 jour), et non à l'instant exact du clic. Une fois close, une saison ne peut **plus** être rouverte.
