...
Fiches profil « page équipe » (pattern apps sportives), atteintes depuis les Classements (et, à terme, la fiche équipe publique #28 via « Voir nos confrontations »). Rendu **pixel-perfect** de la maquette (écrans `opp` / `map`), cartes translucides mutualisées (`ui/stats/MKStatCard.kt` : `StatCard`, `StatHeaderCard`, `BalanceCard`, `WinTieLossBar`, `StatTiles` — extraites de `StatsFullScreen`, rule 16). **12p uniquement**, données réelles. Chaque fiche présente en outre **toutes les données détaillées** de l'écran Statistiques scopées à son entité (cf. « Sections détaillées » ci-dessous).

- **Fiche adversaire** (`OpponentDetailScreen`, route `Opponent/{teamId}`, `OpponentDetailViewModel`) : en-tête (nom/tag du roster + avatar de l'équipe, rule 12 ; nb de confrontations + dernière rencontre), **Bilan face à eux** (winrate + V/N/D + barre), **5 dernières face à eux** (pastilles V/N/D, plus récente à droite), **Séries & scores** (série en cours, record de victoires vs eux, score moyen pour/contre — points, pénalités incluses), **Meilleur circuit contre eux** (par winrate, seuil `MIN_RANKING_SAMPLE`), **sections détaillées** (cf. ci-dessous), **Historique des wars** (`WarCell` → `WarDetailsScreen`). Réutilise le calcul partagé `withFullStats(teamId=…)`.
- **Fiche circuit** (`MapDetailScreen`, route `Map/{trackIndex}`, `MapDetailViewModel`) : en-tête (illustration + nom + « joué N fois » + icône de coupe), **Performance** (winrate de manche + V/N/D + barre), **Scores moyens** (score équipe / position moyenne), **Top 6 / Bot 6** (places de l'équipe sur ce circuit : manches à 6 positions ≤ 6 / ≥ 7), **Meilleur pilote ici** (winrate perso le plus élevé sur le circuit, seuil `MIN_RANKING_SAMPLE`, départage par score perso moyen), **sections détaillées** (cf. ci-dessous). S'appuie sur `MapStats`.

**Sections détaillées communes aux deux fiches** (`ui/stats/MapStatsSections.kt`, `mapStatsDetailSections(MapStats)`) — mêmes calculs et rendus que l'écran Statistiques `StatsFullScreen`, scopés à l'adversaire ou au circuit courant : **Répartition des positions** (histogramme P1→P12 des positions de l'ÉQUIPE + pied Top6/Bot6 %, composant mutualisé `DistributionChart`/`DistributionFooter`), **Top / Bot** (compteurs Top 2→6 et Bot 2→6, `TopBottomColumns`), **Shocks** (nombre total d'objets éclair subis). Chaque section est masquée si vide. Elles lisent un `MapStats` construit sur l'ensemble des manches concernées (`positionDistribution` / `topsTable` / `bottomsTable` / `shockCount`).

Le routage par type se fait dans `RootScreen` (`onStats` dispatche `OpponentStats`→`Opponent/{teamId}`, `MapStats`→`Map/{trackIndex}` CSV, autres→`Stats`).

**Écart résiduel documenté** : pas de nom de coupe en ressource (seule l'icône de coupe existe) → l'en-tête circuit affiche l'icône + « coupe » plutôt que « coupe Champignon ». Pas de flèche `←` visible dans l'app bar (`BaseScreen` n'en propose pas) : le retour se fait par le geste/bouton système (`BackHandler`), comme les autres écrans de détail (`StatsFullScreen`, `WarDetailsScreen`).
