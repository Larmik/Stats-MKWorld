...
| **Shocks/war** | Nombre moyen d'éclairs **joués** par war (les tiens en Indiv). | Mesurer la qualité du **bagging** : farmer puis placer des éclairs (en restant volontairement derrière pour tirer de meilleurs objets) est un rôle à part entière, **aussi important que jouer devant**. | Une moyenne (« 1,5 ») ; plus c'est haut, plus le joueur place d'éclairs. | Pour un rôle de bagueur, **plus d'éclairs = mieux**. À **ne pas** relier à la position finale : bag + éclair n'empêche pas de finir dans le top. | Repérer/évaluer les bagueurs de l'équipe et répartir les rôles (bagging vs front-running). | L'éclair n'entre pas dans le calcul du score ; **aucune corrélation shocks ↔ position finale**. Son delta s'affiche sans couleur (le « mieux » dépend du rôle du joueur). |
...
| **Shocks joués / par war (face à eux)** | Nombre d'éclairs **joués** contre cet adversaire (total et moyenne par war). | Mesurer le **bagging** sur les confrontations contre eux. | Un total + une moyenne. | Rôle de bagueur : plus d'éclairs = mieux ; indépendant de la position finale. | Évaluer les bagueurs sur les matchs contre cette équipe. | Hors calcul du score ; aucune corrélation avec la position finale. |
