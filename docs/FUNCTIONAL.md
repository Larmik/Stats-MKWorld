...
**Sélecteur Indiv / Équipe** (les deux fiches) — `MKSegmentedSelector` partagé (rule 15), état **réactif** du ViewModel (rule 11 : `MutableStateFlow` basculé par `onModeChange`, l'écran reste monté, pas de re-navigation). Libellés contextualisés : adversaire « Rencontrés par le joueur » / « par l'équipe » ; circuit « Joués par le joueur » / « par l'équipe ». **Mode initial semé par le contexte d'ouverture** : `OpponentStats`/`MapStats` portent un `userId` (nullable) passé dans la route (`…/{userId}`, « null » = Équipe) ; toutes les sections réagissent au mode (Indiv = données du joueur courant, Équipe = données de l'équipe hôte).

- **Fiche adversaire** (`OpponentDetailScreen`, route `Opponent/{teamId}/{userId}`, `OpponentDetailViewModel`) : en-tête (nom/tag du roster + avatar de l'équipe, rule 12 ; nb de confrontations + dernière rencontre), **Bilan face à eux** (winrate + V/N/D + barre), **5 dernières face à eux** (pastilles V/N/D, plus récente à droite), **Séries & scores** (série en cours, record de victoires vs eux, **différence de score moyenne** — pour − contre, signée, pénalités incluses — et **shocks joués**), **Circuits contre eux** (podium Top3/Flop3 par score moyen + « **Voir le classement en entier** » → `OpponentTracksRankingScreen`), **sections détaillées** (répartition des positions, Top/Bot 2→6), **Historique des wars** (`WarCell` → `WarDetailsScreen`). Réutilise `withFullStats(teamId=…, userId=…)`.
- **Fiche circuit** (`MapDetailScreen`, route `Map/{trackIndex}/{userId}`, `MapDetailViewModel`) : en-tête (illustration + nom + « joué N fois » + icône de coupe), **Performance** (winrate de manche + V/N/D + barre), **Scores moyens** (score équipe/perso · **position moyenne réelle** selon le mode · **shocks joués**), **sections détaillées** (répartition des positions, Top/Bot 2→6), **Pilotes sur ce circuit** (podium Top3/Flop3 **du meilleur au pire score perso moyen** + « **Voir le classement en entier** » → `MapPilotsRankingScreen`). S'appuie sur `MapStats`.

**Sections détaillées communes** (`ui/stats/MapStatsSections.kt`, `mapStatsDetailSections(MapStats)`) — mêmes calculs/rendus que `StatsFullScreen`, scopés à l'entité ET au mode : **Répartition des positions** (histogramme P1→P12 — positions du joueur en Indiv, de l'ÉQUIPE en Équipe — + pied Top6/Bot6 %, `DistributionChart`/`DistributionFooter`) et **Top / Bot** (compteurs Top 2→6 et Bot 2→6, `TopBottomColumns`). Masquées si vides. (Les **shocks joués** ne sont plus une section autonome : intégrés à « Séries & scores » de l'adversaire et aux « Scores moyens » du circuit.)

**Classements complets** (« Voir le classement en entier ») : `OpponentTracksRankingScreen` (circuits contre l'adversaire, par score moyen) et `MapPilotsRankingScreen` (pilotes sur le circuit, par score perso moyen). Rendus via la **grille de podiums mutualisée** `ui/stats/PodiumGrid.kt` (`podiumRows`, extraite de `StatsRankingScreen`, rule 16), en réutilisant le **même ViewModel** que la fiche (mêmes données, même mode).
...
**Note calcul** : distinction stricte **score vs position** pour les circuits — la « position moyenne » est la position réelle (1..12) moyenne (du joueur en Indiv via `MapStats.averagePlayerPosLabel`, de l'équipe en Équipe via `MapStats.teamAveragePosition`), jamais un score. Les **shocks** sont les objets éclair **joués** (`Shock.playerId` = joueur qui joue l'éclair ; filtrés par joueur en Indiv, tous les joueurs de l'équipe hôte en Équipe).

**Écart résiduel documenté** : pas de nom de coupe en ressource (seule l'icône existe) → en-tête circuit = icône + « coupe ». Pas de flèche `←` visible dans l'app bar (`BaseScreen` n'en propose pas) : retour par geste/bouton système (`BackHandler`), comme `StatsFullScreen`/`WarDetailsScreen`.
