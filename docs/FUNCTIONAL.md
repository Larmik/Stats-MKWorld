...
Ce tableau reprend **chaque statistique de l'application** (vues Individuel /
Équipe, adversaires, circuits, classements) avec, pour chacune : de quoi on parle,
à quoi elle sert, comment la lire, comment l'interpréter vite, un usage concret et
les points à garder en tête. Sauf mention contraire, tout se lit du point de vue de
**ton équipe** (ou de **toi** en vue joueur) et porte sur les wars **12 joueurs**
(le moteur de calcul est déjà prêt pour le 24p, l'affichage viendra dans un ticket
dédié). Rappels : **war** = un match ; **manche** (« track ») = une course d'une
war ; **winrate** = pourcentage de wars gagnées ; « **all-time** » = sur tout
l'historique.

| Nom de la stat | De quoi on parle | À quoi ça sert | Comment la lire | Interprétation rapide | Usage concret | Infos complémentaires |
|---|---|---|---|---|---|---|
| **Marge moyenne de victoire / de défaite** | Pour chaque war, une « marge » = ton score − le score de l'adversaire (pénalités comprises des deux côtés ; en 24p, l'adversaire retenu est celui au meilleur score parmi les 3 équipes). L'app sépare les wars en deux paquets et fait deux moyennes distinctes : marge moyenne de victoire (wars gagnées) et marge moyenne de défaite (wars perdues, en valeur absolue). Les égalités (marge = 0) ne comptent dans aucune des deux. | Répondre à une question que le simple ratio V/D ne dit pas : quand ça se passe bien ou mal, à quel point ? Deux équipes au même bilan peuvent avoir un profil très différent (gagner/perdre serré vs écraser/se faire démolir). | Affichée avec un signe : « +18 » = quand tu gagnes, tu gagnes en moyenne de 18 points ; « -12 » = quand tu perds, tu perds en moyenne de 12 points. | Marge de victoire élevée = victoires larges (tu domines quand tu es devant). Marge de défaite basse (proche de 0) = défaites serrées (tu passes près de gagner). Idéal = grosse marge de victoire + petite marge de défaite ; l'inverse trahit un manque de régularité. | Comparer les deux au taux de victoire : bon winrate mais grosse marge de défaite = équipe qui « prend cher » sur ses rares mauvais jours → cible ce qu'il faut stabiliser. | Division entière (pas de décimale), chaque war pèse pareil ; sur peu de wars, une seule war très large/serrée tire fortement la moyenne. Consultable sur les 5 ou 10 dernières wars pour la tendance récente. |
| **Wars jouées** | Nombre total de wars comptabilisées — en vue joueur, celles où le joueur a participé ; en vue équipe, toutes les wars de l'équipe. | Donner la taille de l'échantillon derrière toutes les autres stats. | Un simple compteur (« 42 wars »). | Plus il est grand, plus les moyennes et pourcentages sont fiables ; un petit nombre invite à la prudence. | Vérifier qu'une stat impressionnante ne repose pas sur 2-3 wars seulement. | Sert de dénominateur au winrate ; en vue joueur, ne compte que les wars où le joueur figure. |
| **Bilan Victoires / Nuls / Défaites (V/N/D)** | Décompte des issues de wars. 12p : victoire = ton score > adverse, nul = égalité exacte, défaite = score < adverse (pénalités incluses). 24p : victoire = hôte dans le top 2 des 3 équipes, défaite = bottom 2. | Photo d'ensemble du palmarès. | Trois nombres, souvent accompagnés d'une barre colorée (vert / gris / rouge). | Le rapport V vs D résume la réussite ; les nuls sont rares en 12p. | Base du winrate ; comparer périodes ou adversaires. | L'issue repose sur le score final avec pénalités, pas sur les seules positions. |
| **Winrate (taux de victoire)** | Pourcentage de wars gagnées = victoires × 100 / wars jouées. | Mesure synthétique du niveau de réussite. | Un pourcentage (« 58 % »). | Plus haut = mieux. 50 % = autant de V que de D. À pondérer par le nombre de wars. | Comparer joueurs, périodes (all-time / 5 / 10 dernières) ou adversaires. | Les nuls comptent au dénominateur mais pas comme victoires : ils tirent donc le winrate vers le bas. |
| **Score moyen par war** | En vue joueur : moyenne des points marqués par le joueur sur une war. En vue équipe : l'écart de score moyen, affiché en différentiel « +X / -X » (par rapport à l'équilibre). | Mesurer le rendement moyen. | Joueur = points bruts (« 62 »). Équipe = écart signé (« +34 » = on marque 34 de plus que l'adversaire en moyenne). | Joueur : plus haut = mieux. Équipe : positif = on domine en moyenne, négatif = on subit. | Suivre la progression sur les fenêtres 5 / 10 dernières. | L'écart équipe se lit sur une échelle de différentiel (points au-dessus/en dessous de l'équilibre), pas en points bruts. |
| **Position moyenne (joueur) / Score moyen par manche (équipe)** | Joueur = place moyenne du joueur sur une manche, reconvertie en position (ex. « P4 »). Équipe = points moyens de l'équipe par manche, en différentiel. | Mesurer la performance à l'échelle d'une seule course. | Joueur = une position ou une plage (« P3-P4 »). Équipe = un écart (« +6 »). | Position → plus BAS = mieux (P1 est le top). Score de manche équipe → plus haut = mieux. | Repérer si un joueur finit régulièrement devant, ou si l'équipe grignote manche après manche. | En vue joueur c'est la position ; en vue équipe/adversaire ça devient le score par manche (l'un remplace l'autre). |
| **% de manches gagnées (Maps gagnées)** | Part des manches où l'équipe a « pris » la manche = score d'équipe de la manche > 41 (moitié haute du barème 82 en 12p). | Mesurer la régularité manche par manche, indépendamment du résultat final de la war. | Un pourcentage. | Plus haut = mieux ; complète le winrate (on peut perdre une war en gagnant la majorité des manches, et inversement). | Détecter une équipe qui gagne beaucoup de manches mais perd les wars serrées (gestion de fin de match). | Seuil à 41 en 12p ; une manche pile à 41 n'est pas comptée gagnée. |
| **Shocks (total / par war)** | Nombre d'objets éclair (shocks) pris : total sur l'historique, ou moyenne « shocks/war ». En vue joueur, filtré sur les shocks du joueur. | Suivre l'usage/subissage des éclairs — indicateur de jeu, pas de score. | Un compte (« 18 ») ou une moyenne décimale (« 1,5 shocks/war »). | Volontairement NEUTRE : ni « haut = bien » ni « bas = bien », à interpréter selon le contexte. | Corréler avec des contre-performances de manche. | Le « shock » est un objet (item) : il n'entre PAS dans le calcul du score ; dans « Forme récente », son delta est affiché sans couleur (direction ambiguë). |
| **Série en cours** | Nombre de wars consécutives gagnées OU perdues à partir de la plus récente ; une war nulle ou un résultat inverse remet à zéro. | Capturer la dynamique du moment. | « +3 victoires », « -2 défaites » ou « Aucune ». | Série positive = bonne passe ; négative = mauvaise passe. | Prendre le pouls avant/après une session. | Calculée dans l'ordre chronologique réel (par date) ; un seul nul casse la série. |
| **Record de victoires** | La plus longue série de victoires consécutives jamais réalisée. | Repère historique du meilleur passage. | « X victoires ». | Plus c'est grand, plus la meilleure passe a été longue. | Objectif à battre. | Sur tout l'historique, en ordre chronologique. |
| **Record de défaites** | La plus longue série de défaites consécutives subie. | Repère du pire passage. | « X défaites ». | Plus c'est grand, plus il y a eu un creux prolongé. | Contextualiser une mauvaise passe actuelle vs le pire déjà vécu. | Sur tout l'historique, en ordre chronologique. |
| **Invaincu depuis (série d'invincibilité)** | Depuis combien de wars tu n'as plus perdu, en comptant victoires ET nuls ; s'arrête à la première défaite. | Visualiser une bonne passe même avec des nuls. | « X wars » (affiché si > 0). | Plus c'est long, mieux c'est ; plus permissif que la série de victoires (les nuls ne cassent pas). | Mettre en avant une régularité récente sans défaite. | Différent de la « Série en cours », qui, elle, casse sur un nul. |
| **Nombre de Top6** | Manches où les 6 joueurs occupent EXACTEMENT les positions 1 à 6 (score d'équipe de la manche = 61). | Compter les manches parfaites. | Un compte de manches (affiché si > 0). | Plus il y en a, plus l'équipe a verrouillé des manches entières. | Repérer une domination totale sur certaines manches/circuits. | Cas EXACT (61 pile) : une manche à cheval haut/bas n'est ni Top6 ni Bot6. 12p. |
| **Nombre de Bot6** | Manches où les 6 joueurs occupent EXACTEMENT les positions 7 à 12 (score d'équipe = 21). | Compter les manches complètement ratées. | Un compte de manches (affiché si > 0). | Plus il y en a, plus l'équipe s'est fait dominer sur des manches entières. | Identifier des circuits à travailler. | Cas EXACT (21 pile). 12p. |
| **Contribution du joueur (%)** | Pourcentage moyen des points de l'équipe apportés par le joueur, calculé war par war (part = score joueur / score équipe) puis moyenné. | Situer le poids du joueur dans le collectif. | « 20 % » = le joueur marque en moyenne un cinquième des points de l'équipe. | Repère : dans une équipe de 6, ~16-17 % = contribution « moyenne » ; nettement au-dessus = joueur moteur. | Comparer les contributions au sein du roster (voir « Contributeurs » en vue équipe). | Visible seulement en vue joueur ; c'est une moyenne de ratios par war (pas la somme des points du joueur divisée par la somme d'équipe). |
| **Régularité — écart-type** | Dispersion des scores autour de leur moyenne (écart-type de population). | Mesurer la constance des performances. | « ± X points ». | Bas = régulier (scores proches d'une war à l'autre) ; haut = irrégulier (des hauts et des bas). | Distinguer un joueur constant d'un joueur « montagnes russes » à winrate égal. | Nécessite au moins 2 wars, sinon non affiché. |
| **Régularité — amplitude (min – max)** | Pire et meilleur score enregistrés sur l'historique. | Visualiser l'écart entre les deux extrêmes. | « X – Y points » (min à gauche, max à droite). | Écart large = performances très variables ; resserré = régulier. | Complément visuel de l'écart-type. | Ce sont des valeurs extrêmes : une seule war peut fixer le min ou le max. |
| **Performance 1ʳᵉ vs 2ᵉ moitié de war (joueur)** | Compare la position moyenne du joueur sur la première moitié des manches vs la seconde moitié. | Voir si le joueur démarre fort et faiblit, ou monte en puissance. | Deux positions (« 1ʳᵉ moitié P4 / 2ᵉ moitié P6 »). | Position qui se dégrade (chiffre qui monte) = fin de war plus faible ; l'inverse = montée en régime. | Ajuster la gestion de concentration/énergie sur la durée d'une war. | La war est coupée en deux (nb de manches / 2) ; vue joueur uniquement ; position → plus bas = mieux. |
| **Points perdus en pénalités** | Total des points retirés à ton équipe par des pénalités sur tout l'historique. | Chiffrer le « coût » cumulé des pénalités. | « -X points » (affiché si > 0). | Plus c'est haut, plus les pénalités ont pesé. | Argument pour réduire les comportements pénalisés ; suivable sur 5 / 10 dernières. | Ne compte que les pénalités de ton équipe (pas celles infligées à l'adversaire). |
| **Forme récente (3 fenêtres + delta)** | Vue de référence comparant 3 fenêtres — all-time, 5 dernières, 10 dernières wars — sur les mêmes indicateurs (winrate, score moyen, position/score par manche, % manches gagnées, shocks/war). | Voir si la forme actuelle est meilleure ou moins bonne que d'habitude. | Une ligne par indicateur, trois valeurs ; les fenêtres récentes affichent un delta (chiffre signé + flèche + couleur) vs l'all-time. | Sens du delta selon l'indicateur : winrate / % manches / score → hausse = vert (mieux) ; position moyenne → baisse = vert (mieux, couleur inversée) ; shocks/war → neutre (sans couleur). | Diagnostiquer une bonne/mauvaise passe récente d'un coup d'œil. | Si moins de wars que la fenêtre demandée, l'app affiche le disponible et n'affiche pas de delta trompeur. |
| **Distribution des positions (joueur)** | Mini-histogramme du nombre de fois où le joueur a fini à chaque position (P1→P12 en 12p, P1→P24 en 24p). | Voir la répartition des résultats du joueur. | Une barre par position ; barre haute = position fréquente. | Concentré sur P1-P3 = joueur de tête ; étalé = résultats dispersés/irréguliers. | Identifier un profil (souvent devant vs en dents de scie) ; consultable sur 5 / 10 dernières. | Un pied de carte résume aussi le % de manches finies en Top6 / Bot6. Vue joueur uniquement. |
| **Meilleurs / pires circuits — par winrate** | Top 3 et flop 3 des circuits selon le pourcentage de manches gagnées dessus. | Repérer où l'équipe/le joueur performe ou coince. | Trois circuits en haut (meilleur winrate), trois en bas (pire), avec winrate ET score moyen affichés côte à côte. | Winrate élevé = circuit « fort » ; faible = circuit à travailler. | Choisir/bannir des circuits, orienter l'entraînement. | Un circuit n'apparaît qu'à partir de 3 matchs joués (échantillon jugé fiable). |
| **Meilleurs / pires circuits — par score moyen** | Top 3 et flop 3 des circuits selon les points moyens marqués dessus (score joueur en vue joueur, score équipe en vue équipe). | Compléter le winrate par le rendement en points. | Mêmes lignes, critère « score moyen ». | Un circuit peut être gagné souvent mais de peu (bon winrate, score moyen modeste) : les deux critères se complètent. | Distinguer les circuits qu'on gagne largement de ceux qu'on gagne de justesse. | Seuil de 3 matchs également. |
| **Meilleurs / pires adversaires — « winrate face à »** | Top 3 et flop 3 des équipes contre lesquelles on gagne le plus / le moins souvent. | Identifier bêtes noires et adversaires favoris. | Chaque adversaire sur une ligne pleine largeur : wars jouées + winrate + score moyen. | Winrate faible « face à » = adversaire difficile. | Préparer un match selon l'historique face à l'équipe visée. | Seulement les adversaires rencontrés ≥ 3 fois. Disponible en vues équipe ET joueur (côté joueur = de son point de vue). Libellé volontairement « face à ». |
| **Meilleurs / pires adversaires — « score moyen face à »** | Top 3 et flop 3 des équipes contre lesquelles on marque le plus / le moins de points en moyenne. | Nuancer le winrate par l'écart de points. | Même cellule, critère « score moyen ». | Bon winrate mais faible score moyen face à une équipe = matchs serrés. | Repérer les adversaires contre qui on gagne « large » vs « au finish ». | Seuil de 3 confrontations ; vues équipe ET joueur. |
| **Différence de score moyenne (fiche adversaire)** | Écart de score moyen face à un adversaire donné (score équipe − score adverse, moyenné sur les confrontations). | Résumer la domination/subordination face à cette équipe. | Un écart signé (« +22 »). | Positif = on marque plus qu'eux en moyenne ; négatif = l'inverse. | Jauger le rapport de force réel au-delà du seul winrate. | Sur la fiche détail de l'adversaire. |
| **5 dernières confrontations (pastilles V/N/D)** | Les 5 dernières wars face à un adversaire, en pastilles colorées. | Tendance récente contre cette équipe précise. | 5 points vert/gris/rouge, du plus ancien au plus récent. | Suite de verts = ascendant pris ; de rouges = domination subie. | Contexte immédiat avant un match. | Sur la fiche détail de l'adversaire. |
| **Tables Top N / Bot N d'équipe (2 à 6)** | Pour N de 2 à 6, nombre de manches où les N meilleures (resp. pires) positions de l'équipe sont regroupées dans le haut (resp. le bas) du classement. | Nuancer entre le Top6 parfait et des regroupements partiels. | « Top 6 : 5, Top 5 : 8… » (et l'équivalent Bot). | Beaucoup de Top4/5 = l'équipe place souvent un gros paquet devant sans forcément verrouiller les 6. | Analyse fine de la répartition d'équipe par manche. | Sur les fiches détail (adversaire/circuit). Bot N = positions basses symétriques (Bot6 ⇒ ≥ 7…). |
| **Positions individuelles P1→P12 (tables détail)** | Nombre de fois où le joueur a fini exactement à chaque position, décliné en tables 1→6 et 7→12. | Détail chiffré derrière l'histogramme de distribution. | « 1 : 3, 2 : 5… ». | Concentration sur les petites positions = joueur de tête. | Analyse fine par circuit ou par adversaire. | Vue joueur ; sur les fiches détail. |
| **Classement des contributeurs (équipe)** | Classement du roster par part de points apportés (%) sur la fenêtre choisie, avec le winrate de chacun. | Voir qui porte l'équipe. | Liste ordonnée : rang, % des points, winrate. | Écarts marqués = équipe portée par 1-2 joueurs ; parts équilibrées = collectif homogène. | Équilibrer les rôles, valoriser les moteurs. | Vue équipe uniquement ; calculé par fenêtre (all-time / 5 / 10 dernières). |
| **Classements (Joueurs / Adversaires / Circuits)** | Écran dédié listant joueurs, adversaires ou circuits, triables et filtrables. | Comparer toutes les entités entre elles, pas seulement un top 3. | Recherche par nom + 3 tris (nb de matchs / winrate / score moyen) + curseur d'occurrences minimum. | Le tri choisi définit « le meilleur » ; le curseur écarte les petits échantillons. | Trouver le meilleur joueur par winrate, l'adversaire le plus fréquent, le circuit le plus joué… | Adversaires/circuits en perspective équipe ; joueurs sectionnés Membres / Alliés. Clic → fiche détail. |
