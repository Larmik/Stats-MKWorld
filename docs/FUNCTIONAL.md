...
> **Le retour en arrière annule la sélection de l'étape rejointe** (demande utilisateur) : revenir à **Adversaire** (« Précédent », back système ou clic stepper) **désélectionne l'adversaire** (les adversaires en 24p) et **réaffiche la liste complète** des équipes ; revenir aux **Joueurs** depuis le Récap **remet la line-up à zéro** (aucun joueur coché). Aller **en avant** ne réinitialise rien. Le gating du stepper empêche alors de re-remonter au Récap sans re-compléter la sélection.
