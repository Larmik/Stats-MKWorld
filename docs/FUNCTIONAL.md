...
3. **Historique complet**, groupé par mois (en-têtes collants), triés du plus récent au plus ancien. **Tous les modes sont mélangés (12j ET 24j)** — l'ancien filtrage par mode (`is24PEnabled`) a été retiré. **La war EN COURS (live) est exclue de l'historique** : la liste ne contient que les wars terminées (la war en cours reste accessible via la bannière « Reprendre » en tête, pas dans la liste). Clic sur une war → détail de war. Réutilise la cellule `WarCell` unifiée.

> **Variante « historique filtré par joueur » (#65)** : `WarListScreen` accepte un `userId` optionnel (`WarListViewModel.Factory.create(userId)`). Sans `userId` (pôle Wars) → historique complet de l'équipe (filtre roster hôte seul). Avec `userId` (route `Home/WarList/{userId}`, ouverte par le lien « Résultats → » des stats) → seules les wars où **ce joueur a joué** (`War.hasPlayer`, participation à toutes les courses) ; le sous-titre devient « Wars de <joueur> · N ». `userId = « me »` résout le **joueur courant** (id MKCentral en DataStore). Même écran, mêmes chips/cellules (rule 16), un seul exemplaire. **L'exclusion de la war en cours vaut dans les deux cas** (pôle Wars et historique filtré).
