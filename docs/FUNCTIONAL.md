...
1. **Joueurs** — chips `Wars (défaut) / Winrate / Score moy.`. Liste **sectionnée** en **Membres** (joueurs de l'équipe) et **Alliés** (deux en-têtes). Cellule = **médaillon joueur** (photo de profil MKCentral si dispo, sinon initiales, #50 pt.4) + nom + **4 lignes** de stats : Wars jouées, **Taux de participation** (#78, `wars jouées ÷ wars de l'équipe × 100` sur la saison filtrée, sous « Wars jouées »), Winrate, Score moy. Ligne → stats joueur (`StatsType.PlayerStats`).
