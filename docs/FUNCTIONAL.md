...
> **Saison en cours (#70)** : la carte de salutation affiche une **pastille « Saison N »** sous le sous-titre profil (numéro de la saison sans date de fin, lu depuis la table `seasons` de #30). ⚠️ **Écart assumé vs prototype (rules 13/15)** : la maquette `home` ne place PAS d'indicateur de saison sur l'accueil — c'est un ajout de ce ticket, placé dans le style Accueil (pastille translucide). La pastille est masquée tant qu'aucune saison en cours n'est hydratée.
...
>
> **Filtrage par saison (#70)** : un **sélecteur de saison** (`MKSegmentedSelector` partagé) est placé **tout en haut** de l'écran, au-dessus du sélecteur de période. Options : **« Tout l'historique »** puis une entrée par saison (« Saison N »). Défaut = **saison en cours**. Au changement, **toutes** les sections se recalculent **à la volée** sur les seules wars de la saison sélectionnée (intervalle `[start, end]` comparé au timestamp `war.id`) — dynamiquement, sans re-navigation (rule 11). Filtre et période sont **indépendants** (saison = quelles wars ; période = les N dernières de cette saison).
>
> **Libellés de saison sur les records (#70)** : en vue **« Tout l'historique » ET période all-time**, les tuiles de « Records & séries » (amplitude min/max, record V, record D) affichent en légende la **saison où le record a été établi** (« S2 »), résolue par le timestamp de la war du record. En vue **saison précise** (courante ou passée) ou sur une **période 5/10**, ce libellé est **omis** (redondant/non pertinent). Top6/Bot6 (agrégats, pas des records datés) n'ont pas de libellé.
