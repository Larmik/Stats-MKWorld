...
- **Sélecteurs de section** — état UI hissé dans `StatsFullScreen` (`rememberSaveable`, rule 11) et passé aux sections via `SectionSelectors` (les sections sont des extensions `LazyListScope`) :
  - **Fenêtre Indicateurs** (all-time / 5 / 10) : la carte lit `allTimeForm`/`recentForm5`/`recentForm10` et affiche valeur + **delta % vs all-time** (flèche ↗/↘ colorée, uniquement sur le delta ; valeurs en blanc). `FormStats` a été **étendu** pour porter par fenêtre `scoreStdDev`, `scoreMin`/`scoreMax`, `winMargin`/`lossMargin` en plus de winrate/score/position/mapsWon/shocks. En vue **Équipe**, la tuile « Score moyen » affiche l'**écart** (`averageScore.warScoreToDiff(false)`), pas le total ; en **Individuelles**, points/war bruts.
  - **Tri podiums** winrate / score : bascule `topMapsByWinrate`↔`topMapsByScore` (+ flop) et les classements adversaires winrate↔score.
- **Grilles régulières** : `MetricTiles` (Indicateurs & Records) dispose 3 colonnes à poids égal, **hauteur uniforme par ligne** (`Row(height(IntrinsicSize.Min))` + `fillMaxHeight` sur chaque tuile) → toutes les tuiles ont la même taille même avec un delta.
- **Podiums** : « Meilleurs / pires circuits » et « Meilleurs / pires adversaires » en **Top 3 / Flop 3, chacun sur UNE ligne** (`PodiumRow` : 3 cellules à poids égal, `IntrinsicSize.Min`) avec les **cellules historiques** compactes `MapCell` (illustration) et `TeamCell` (logo, `tagVisible = false`). `userId` ⇒ perspective joueur (score du joueur), null ⇒ équipe (écart).
- **Records & séries** : en **tuiles** (`RecordsTilesCard`) — série en cours, puis **amplitude du score** (déplacée depuis les Indicateurs, sous la série ; Top6/Bot6 décalés d'un cran), records V/D, Top6/Bot6, invaincu.
- **Pénalités** : dans les Indicateurs, la tuile Amplitude a été remplacée par « Points perdus en pénalités » (`Stats.penaltyPointsLost`, cumul all-time).
