...
- **Mode Indiv/Équipe** : chaque VM porte un `MutableStateFlow<Boolean> isIndiv` semé par `initialUserId` (assisté) ; le sélecteur (`MKSegmentedSelector` partagé, rule 15) appelle `onModeChange(indiv)`, qui bascule le flow. Le flow des wars est **`combine`** avec `isIndiv` → recalcul complet du `Stats`/`MapStats` scopé au joueur courant (userId = `dataStoreRepository.mkcPlayer.id` en Indiv, `null` en Équipe) sans re-navigation. Le `userId` initial transite par la **route** (`…/{userId}`, littéral « null » = Équipe) — l'argument sème l'état, le toggle bascule ensuite.
- **Cartes partagées** : primitives de carte translucide **extraites** dans **`ui/stats/MKStatCard.kt`** (rule 16) — `StatCard` (+ `titleTrailing` pour le lien « Voir le classement en entier »), `Eyebrow`, `StatHeaderCard`, `BalanceCard`, `WinTieLossBar`, `StatTiles`/`StatTile`, `StatCardRadius`. `StatsFullScreen` importe ces versions publiques. `StatHeaderCard` attend un **logo déjà préfixé** ; les fiches préfixent le `TeamEntity.logo` brut.
- **`OpponentDetailScreen` + `OpponentDetailViewModel`** (route `Opponent/{teamId}/{userId}`, `Factory(teamId, initialUserId)`) : `getWars()` × `isIndiv`, filtre 12p face à l'opposant (+ wars du joueur en Indiv), `withFullStats(teamId, userId)` (V/N/D, séries) ; dérive dernière rencontre, 5 dernières issues, **différence de score moyenne** (`avg(scoreHostWithPenalties − scoreOpponentWithPenalties)`, signée), **shocks joués** (`MapStats.shockCount`), **top3/flop3 + classement complet des circuits contre eux** (`stats.topMapsByScore`/`flopMapsByScore` et `stats.maps` trié par `playerScore`/`teamScore` selon le mode), `MapStats` (sections détaillées), historique. Nom/tag = roster, avatar = équipe (rule 12).
- **`MapDetailScreen` + `MapDetailViewModel`** (route `Map/{trackIndex}/{userId}`, `Factory(trackIndex, initialUserId)`) : `getWars()` × `isIndiv`, `MapStats` scopé (mode) ; **Scores moyens** = score équipe/perso, **position moyenne réelle** (`MapStats.teamAveragePosition` en Équipe, `averagePlayerPosLabel` en Indiv — plus de confusion score/position), **shocks joués** ; **classement des pilotes** sur le circuit (`computePilots`, tous pilotes, **tri par score perso moyen décroissant**) → podium Top3/Flop3 + liste complète. La carte « Top 6 / Bot 6 » highlight a été **supprimée** (décision utilisateur ; seul le Top/Bot 2→6 détaillé subsiste).
- **Sections détaillées mutualisées** (`ui/stats/MapStatsSections.kt`, `LazyListScope.mapStatsDetailSections(MapStats)`, aux deux fiches) : **Répartition des positions** (`ui/stats/MKDistributionCard.kt` : `DistributionChart`/`DistributionFooter` **extraits** de `StatsFullScreen`) et **Top/Bot 2→6** (`ui/stats/MKTopBottomStatsCard.kt` : `TopBottomColumns`). Les **shocks** ne sont plus une section autonome (intégrés à « Séries & scores » / « Scores moyens »). Alimentées par `MapStats.positionDistribution` (**nouvelle** propriété : positions du joueur en Indiv, de l'ÉQUIPE hôte sinon) et `topsTable`/`bottomsTable`. `MapStats.teamAveragePosition` (**nouvelle** propriété) porte la position moyenne d'équipe.
- **Classements complets** (`OpponentTracksRankingScreen`, `MapPilotsRankingScreen`) : réutilisent le **même VM** que la fiche (même clé de nav → mêmes données/mode) et la grille **`ui/stats/PodiumGrid.kt` `podiumRows`** (extraite de `StatsRankingScreen`, rule 16 — ce dernier l'importe désormais). Cartes podium Top3/Flop3 des fiches via **`ui/stats/MKPodiumSectionCard.kt` `PodiumSectionCard`** (mutualisée).
- **Routage** : `RootScreen.onStats` dispatche `OpponentStats`→`Opponent/{teamId}/{userId}`, `MapStats`→`Map/{trackIndex}/{userId}` ; sous-routes `…/Tracks` et `…/Pilots` pour les classements complets. Retour par `BackHandler`.
