...
- **Calcul de stats hors du thread UI (`flowOn(Dispatchers.Default)`, #73)** : les ViewModels qui agrègent les wars (rankings pilotes/adversaires/circuits, fenêtres all-time/5/10, dashboard) faisaient leur calcul lourd sur le collecteur (`viewModelScope` = `Main.immediate`), figeant l'UI au changement de saison (`MKSeasonDropdown` dans les headers Accueil/Wars/Stats/Classements), au basculement de segmented et à la navigation. Correctif : `.flowOn(Dispatchers.Default)` posé sur **la seule branche de calcul** (avant `mergeWith(_state)` / `stateIn`) de `StatsFullViewModel`, `MapDetailViewModel`, `OpponentDetailViewModel`, `StatsRankingViewModel`, `WelcomeViewModel` et `WarListViewModel` — le flux d'état interactif `_state` (tri/onglet/recherche/curseur, léger) reste sur le collecteur. Aucune valeur affichée ne change (rule 13). En complément, `PlayerMapsRankingScreen`/`PlayerOpponentsRankingScreen` mémoïsent le tri **et** la conversion vers `PodiumEntry` via `remember(sortIndex, source, …)` (rule 11) au lieu de les recalculer à chaque recomposition. Cf. rules `.claude/rules/30-repositories.md` (ne pas bloquer), `20` (ordre d'init des flows), `11`.
