...
- **`OpponentDetailScreen` + `OpponentDetailViewModel`** (route `Opponent/{teamId}/{userId}`, `Factory(teamId, initialUserId)`) : `getWars()` × `isIndiv`, filtre 12p face à l'opposant (+ wars du joueur en Indiv), `withFullStats(teamId, userId)`. **Séries & scores** : série en cours, **record série de victoires** (`bestWinStreak`) ET **record série de défaites** (`worstLossStreak`), tuile score **mode-dépendante** (Équipe = `avg(scoreHostWithPenalties − scoreOpponentWithPenalties)` signée ; Indiv = `stats.averagePoints` = score moyen du joueur car `withFullStats(userId)` met `warScores` = points du joueur), **shocks joués** `MapStats.shockCount` (scopé par mode) affichés « N (ratio) » où ratio = `shockCount / warsPlayed`. Top3/flop3 + classement complet des circuits (`stats.topMapsByScore`/`flopMapsByScore`, `stats.maps` trié par `playerScore`/`teamScore`). Nom/tag = roster, avatar = équipe (rule 12).
- **`MapDetailScreen` + `MapDetailViewModel`** (route `Map/{trackIndex}/{userId}`, `Factory(trackIndex, initialUserId)`) : `getWars()` × `isIndiv`. **Scores moyens** = *score d'ÉQUIPE* + *position du JOUEUR courant* **FIXES** (calculés sur toutes les manches via un `MapStats(userId = currentUserId)` indépendant du mode) + *shocks joués* **DYNAMIQUE** (`mapStats.shockCount` scopé mode). Plus de carte/label **coupe** (retirée). **Classement des pilotes** (`computePilots`, **membres uniquement** — alliés `rosterId == "-1"` exclus —, tri par **score perso moyen décroissant**) → podium Top3/Flop3 + liste complète, **affiché en mode Équipe uniquement** (masqué en Indiv). Sections détaillées mode-scopées.
...
- **Classements complets** (`OpponentTracksRankingScreen` — **texte des cellules en noir** —, `MapPilotsRankingScreen`) : réutilisent le **même VM** que la fiche (même clé de nav → mêmes données/mode) et la grille **`ui/stats/PodiumGrid.kt` `podiumRows`** (extraite de `StatsRankingScreen`, rule 16). Cartes podium Top3/Flop3 des fiches via **`ui/stats/MKPodiumSectionCard.kt` `PodiumSectionCard`** (mutualisée).
