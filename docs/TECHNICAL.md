...
- **Mémoïsation des tris/podiums des classements complets (#73)** : `PlayerMapsRankingScreen`/`PlayerOpponentsRankingScreen` mémoïsent le tri **et** la conversion vers `PodiumEntry` via `remember(sortIndex, source, …)` (rule 11) au lieu de les recalculer à chaque recomposition. **Le déport du calcul de stats hors du thread UI (offload) a été tenté puis REVERTÉ** dans le cadre de #73 : `.flowOn(Dispatchers.Default)` **et** `withContext(Dispatchers.Default)` ciblé ont **tous deux** fait **disparaître le sélecteur de saison (`MKSeasonDropdown`) de tous les headers**. Que même un `withContext` ciblé (qui préserve l'ordre d'émission et garde `seasons` peuplé dans le `State`) casse l'affichage indique une **affinité au thread principal non résolue** dans la chaîne de calcul (`withFullStats`/`computeState`/`computeRankings`) : un throw runtime off-main tue la branche `combine`/`map` → `stateIn` reste sur son seed vide → `seasons = []` → dropdown masqué. Régression **runtime** (invisible à `compileDebugKotlin`). Le calcul reste donc **sur le collecteur** (comportement d'origine) tant que l'affinité n'est pas identifiée/corrigée et **vérifiée sur device** — cf. **rule `.claude/rules/21-vm-offload-compute.md`**.
