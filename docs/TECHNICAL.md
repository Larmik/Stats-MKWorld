...
- **Correction V/N/D & compteur de wars (#36)** : le décompte vient de `Stats.warStats` désormais calculé sur la liste **filtrée** (cf. §9.4) → en Individuelles il ne compte que les wars du joueur, en Équipe toutes celles de l'équipe. Distinction stricte indiv/équipe pour chaque indicateur.
- **Sélecteurs de section (#36)** — état UI hissé dans `StatsFullScreen` (`rememberSaveable`, rule 11) et passé aux sections via `SectionSelectors` (les sections sont des extensions `LazyListScope`) :
  - **Fenêtre Indicateurs** (all-time / 5 / 10) : la carte « Indicateurs » lit `allTimeForm`/`recentForm5`/`recentForm10` et affiche valeur + **delta % vs all-time** (flèche ↗/↘ colorée, uniquement sur le delta ; valeurs en blanc). `FormStats` a été **étendu** (#36) pour porter par fenêtre `scoreStdDev`, `scoreMin`/`scoreMax` (amplitude), `winMargin`/`lossMargin` en plus de winrate/score/position/mapsWon/shocks.
  - **Tri podiums** winrate / score : bascule `topMapsByWinrate`↔`topMapsByScore` (+ flop) et les classements adversaires winrate↔score.
- **Podiums (#36)** : « Meilleurs / pires circuits » et « Meilleurs / pires adversaires » sont rendus en **Top 3 / Flop 3** avec les **cellules historiques** `MapCell` (illustration + infos, perspective `userId`/`is24p`) et `TeamCell` (logo, `userId` ⇒ score joueur vs écart équipe). Remplacent les tuiles « le + joué / meilleur / pire » et la section « Tes circuits » (supprimée).
- **Records & séries (#36)** : restylés en **tuiles** (même vocabulaire que la carte Indicateurs) via `RecordsTilesCard` (série en cours, records V/D, Top6/Bot6, invaincu).
- **Distribution des positions (#36)** : barres dans une **zone à hauteur fixe** ancrées `BottomCenter` (ligne de base commune) → les labels de position sous les barres sont **alignés horizontalement**.
- **Aucune perte vs l'ancien `StatsScreen`** : les indicateurs de l'ancien écran sont couverts par les sections ci-dessus ; la carte accordéon `MKAdvancedStatsCell` est conservée en fin d'écran pour les indicateurs résiduels (invaincu, points perdus en pénalités). La distribution est rendue par le graphe maquette (`DistributionChart`), pas `MKPositionDistributionCell` (doublon évité).
- **Rendu pixel-perfect** (rules 13/15) : cartes translucides, eyebrows, tuiles, barre V/N/D proportionnelle, flamme de séries, histogramme P1→P12 + pied Top6/Bot6, comparatif 12/24 en deux colonnes (mode courant surligné), contributeurs, podiums. En-tête : vignette photo joueur / logo équipe via `AsyncImage` (fallback initiales/`default_logo`). **Pas de pastille « Nouveau »** (marqueur de démo, cf. rule 15). **Saisons masquées** (#30 non livré). Nombre de courses retiré de l'en-tête (#36).
