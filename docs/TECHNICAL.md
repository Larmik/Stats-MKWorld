...
**Cellule joueur du Résumé AddTrack** (`SummaryPlayerCell`, privée à `AddTrackScreen` — consommateur unique, rule 61) : reproduit **fidèlement la maquette** `addtrack` (`#at-sum` → `.b`) — carte translucide `white30`/radius 6/padding 11, `Nom · P{n}` (Nunito bold) + ligne « Shocks » avec contrôle `− N +` (boutons carrés 22 dp, `.shk`). Édite les shocks **hors calcul du score**. La grande cellule `PlayerCell` (numéro `MKPosition` + drapeau) n'est plus utilisée ici. **`ui/cells/PositionCell.kt`** expose désormais un paramètre optionnel **`fontSize`** (défaut = rendu historique 70/50 conservé pour `EditTrack` ; `AddTrack` passe une valeur réduite 48/34 pour un rendu plus harmonieux dans sa grille).
