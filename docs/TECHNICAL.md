...
- **Podiums (tri 3 dimensions, Occurrences par défaut)** : « Meilleurs / pires circuits » et « adversaires » en **Top 3 / Flop 3, chacun sur UNE ligne** (`PodiumRow`). Sélecteur `Occurrences | Winrate | Score` (défaut Occurrences) :
  ...
  - **`PodiumCell` (partagée circuit/adversaire)** reprend **toutes** les infos de `MapCell`/`TeamCell` : image en haut (illustration `Maps.picture` arrondie vs logo `AsyncImage`/`default_logo` cercle), nom (2 lignes), puis **lignes de stats empilées** — circuits : *joué N fois*, *winrate*, *score équipe / position joueur* ; adversaires : *N confrontations*, *winrate*, *score équipe (écart) / score joueur*. Les `MapCell`/`TeamCell` historiques (trop larges à 3-par-ligne) ne sont plus utilisées ici.
