...
- **Calcul de stats hors du thread UI (`withContext(Dispatchers.Default)`, #73)** : les ViewModels qui agrègent les wars (rankings pilotes/adversaires/circuits, fenêtres all-time/5/10, dashboard) faisaient leur calcul lourd sur le collecteur (`viewModelScope` = `Main.immediate`), figeant l'UI au changement de saison (`MKSeasonDropdown` dans les headers Accueil/Wars/Stats/Classements), au basculement de segmented et à la navigation. Correctif : la lambda de `combine`/`map` de `StatsFullViewModel`, `MapDetailViewModel`, `OpponentDetailViewModel`, `StatsRankingViewModel`, `WelcomeViewModel` et `WarListViewModel` est structurée en **deux temps** — (a) lectures de sources (Room `getSeasons`/`getWars`, DataStore, Firebase `getCurrentWar`/`listenToCurrentWar`) + résolution du filtre saison + champs légers du `State` (`seasons`, `selectedSeasonNumber`) sur le collecteur ; (b) **seule** la portion CPU lourde (`withFullStats`, `computeState`/`computeRankings`, mapping/tri/`groupBy`, podiums) enveloppée dans `withContext(Dispatchers.Default) { … }`. **Un blanket `.flowOn(Dispatchers.Default)` a d'abord été essayé puis abandonné** : sur une chaîne passant par `mergeWith` (`flowOf(this, flow).flattenMerge()`, merge NON ordonné), `flowOn` transforme l'émission déterministe en course cross-thread où l'état vide (`seasons = []`) peut survivre → **régression : dropdown de saison disparu de tous les headers** ; et il relocalise les lectures de sources (risque d'exception main-affine tuant la branche compute). `withContext` cible le CPU sans toucher aux I/O ni à l'ordre/threading d'émission — cf. **rule `.claude/rules/21-vm-offload-compute.md`**. Aucune valeur affichée ne change (rule 13). En complément, `PlayerMapsRankingScreen`/`PlayerOpponentsRankingScreen` mémoïsent le tri **et** la conversion vers `PodiumEntry` via `remember(sortIndex, source, …)` (rule 11) au lieu de les recalculer à chaque recomposition. Cf. rules `21`, `30` (ne pas bloquer), `20` (ordre d'init des flows), `11`.
...
- **`fetchSeasons(teamId)`** — synchro RTDB → Room, appelée par `FetchUseCase.fetchData` après les wars (rattachée à l'**équipe**, comme `newAllies`/`users` — `team.id`, pas le roster). **Seeding-si-vide** : si `seasons/{teamId}` est vide en RTDB, délègue à `seedInitialSeasons` (écrit l'historique réel + peuple Room) ; sinon rafraîchit simplement le cache Room depuis RTDB (`clearSeasons()` + `writeSeasons(List<SeasonEntity>)`).
