...
4. **Émission** : `flowOf(Stats(WarStats(this, is24p), warScores, maps, averageForMaps, userId))`.
...
- **`StatsFullViewModel`** (`@AssistedInject`, `Factory.create(userId: String?, showTabs: Boolean)`) : `userId` null ⇒ joueur courant (résolu via `dataStoreRepository.mkcPlayer`). Il **réutilise `withFullStats`** (aucun recalcul UI) et calcule, par émission :
  ...
  - tuiles **circuits** (meilleur/pire winrate perso et équipe, le + joué) dérivées de `Stats.bestMapByWinrate`/`worstMapByWinrate`/`mostPlayedMap` ; les libellés de circuit transitent en **`@StringRes`** (`NamedTile.labelRes`) et l'**illustration** (`pictureRes` = `Maps.picture`) sont résolus à l'affichage (Context) ; les adversaires portent le **logo d'équipe** (`NamedTile.logo` = `TeamEntity.logo`) ;
  - `topOpponentsByWinrate`/`flop…`/`…ByScore` (équipe **et** joueur) : top3/flop3 adversaires via `withFullTeamStats` (comme `StatsViewModel`, seuil `MIN_RANKING_SAMPLE`, rule 32) ;
...
- **Calculs ajoutés à `Stats.kt`** (rule 61, dans le récepteur) : `bestCoursePoints` / `worstCoursePoints` (meilleur/pire score du joueur sur une manche, vue joueur) et `mostPlayedMap` (circuit le plus joué). Le reste réutilise l'existant (`allTimeForm`, `positionDistribution`, `firstHalfAvgPosition`/`secondHalfAvgPosition`, `averageWinMargin`/`averageLossMargin`, `playerContribution`, `scoreStdDev`/`scoreMin`/`scoreMax`, `bestWinStreak`/`worstLossStreak`, `unbeatenStreak`, `penaltyPointsLost`, `top6Count`/`bot6Count`, classements maps `topMapsByWinrate`/`…ByScore`, etc.).
- **Aucune perte vs l'ancien `StatsScreen`** : au-delà des sections maquette, l'écran **réintègre les cellules accordéon `ui/stats/*`** que `StatsScreen` rendait pour `PlayerStats`/`TeamStats` — `MKRecentFormCell` (3 fenêtres all-time/5/10), `MKRecordsCell` (séries + records + Top6/Bot6), `MKAdvancedStatsCell` (régularité, amplitude, marges V/D, contribution, 1ʳᵉ/2ᵉ moitié, invaincu, pts pénalités), `MKMapsRankingCell` (top3/flop3 circuits winrate ET score), `MKOpponentsRankingCell` (top3/flop3 adversaires winrate ET score) — reconstruites avec un `StatsType.PlayerStats`/`TeamStats` dérivé de `State.targetUserId` + `is24p`. La distribution des positions est rendue par le graphe maquette (`DistributionChart`), pas par `MKPositionDistributionCell` (doublon évité).
- **Rendu pixel-perfect** (`StatsFullScreen`, rules 13/15) : cartes translucides, eyebrows, tuiles (avec **vignette** logo/illustration), barre V/N/D proportionnelle, flamme de séries, histogramme de distribution P1→P12 avec pied Top6/Bot6, comparatif 12/24 en deux colonnes (mode courant surligné), mini-classement contributeurs. Composants privés au fichier (reprennent le vocabulaire visuel de `WelcomeScreen`). **Pas de pastille « Nouveau »** (marqueur de démo du prototype, cf. rule 15). **Saisons masquées** (#30 non livré) : « record N » sans suffixe saison. Vignettes : logo d'équipe via `AsyncImage("https://mkcentral.com$logo")` (fallback `default_logo`, clip cercle — mécanisme `TeamCell`) ; illustration de circuit via `painterResource(Maps.picture)` (clip arrondi — mécanisme `MapCell`).
