...
- Composants UI maison préfixés `MK` (`ui/MKButton.kt`, `MKText`, `MKDialog`, `MKTextField`, `MKSegmentedSelector`, `MKLoaderDialog`, `MKBottomSheet`…). Cellules de liste dans `ui/cells/`, widgets de stats dans `ui/stats/`. **`MKButton`** est le bouton **unique** de l'app (#50) : **un seul style** aligné sur `.btn2` de la maquette (fond blanc translucide `Colors.white30`, bordure `Colors.whiteBorderSoft`, libellé majuscule Urbanist), celui des boutons de référence « Générer le Tab » / « Voir l'adversaire ». **Il n'y a plus de variante** (l'ancien `MKButtonStyle` `Gradient`/`.cta` **et** `Minor` ont été supprimés — demande utilisateur d'harmonisation, `MKButton` ne prend plus de `style`). **Composant bouton UNIQUE** : l'ancien `WarActionButton` (privé de `WarDetailsScreen`, boutons « Générer le Tab » / « Voir l'adversaire ») a été **fusionné dans `MKButton`** (rule 16) — il n'existe plus de second composant bouton. Params variables (non des variantes de style) : `textColor` (contraste selon le fond : blanc par défaut, `Colors.black` sur surface claire type `MKDialog`) et **`icon: Int?`** (drawable de tête optionnel 16 dp ; avec icône, le rendu reprend exactement l'ancien `WarActionButton` : hauteur 46 dp, padding horizontal 12 dp, libellé fontSize 12 + espace 8 dp ; sans icône, libellé centré fontSize 14). **Divergence assumée vs maquette (rules 13/15)** : le prototype distingue un CTA primaire dégradé (`.cta`) d'un bouton secondaire (`.btn2`) ; cette hiérarchie primaire/secondaire est **volontairement aplatie** à la demande de l'utilisateur (tous les boutons sur le style secondaire). **`MKSegmentedSelector`** est le **segmented unique** de l'app (style « pill » de la maquette) : composant **stateless** (`page` = index sélectionné, `onClick` remonte l'index) avec un paramètre **`onDark`** adaptant les couleurs au fond hôte (`true` = carte sombre `blackAlphaed` → texte inactif blanc ; `false` défaut = fond clair du dégradé `BaseScreen` → texte inactif sombre, lisible). Consommé par Accueil (segmentés Moi/Équipe et 5/10 dans les cartes sombres, `onDark = true`), AddWar (12/24), Annuaire (joueurs/équipes), Stats (individuel/équipe, tri des podiums) et Classements (sous-onglets Joueurs/Adversaires/Circuits **et** chips de tri Winrate/Score/compteur). Ne pas recréer de segmented local (cf. rule `.claude/rules/15-ui-prototype-reference.md`).
