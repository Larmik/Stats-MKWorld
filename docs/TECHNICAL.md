...
- **Lien « Résultats → » de `StatsFullScreen` (#65)** : `BalanceCard(showResultsLink = true)` → **toujours visible** (pôle Stats `showTabs=true` ET fiche `statsfull` `showTabs=false`). Son `onResults` ouvre l'**historique filtré par joueur** via la route **du graphe racine** `Home/WarList/{userId}` (résolvant le crash : l'ancienne cible `Home/WarList` n'existe QUE dans le NavHost interne de `HomeScreen`, introuvable depuis le graphe racine où vit `Statsfull`). Câblage : la route `Statsfull/{userId}` passe `Home/WarList/$userId` (ce joueur) ; le pôle Stats (`Home/Stats`) remonte au graphe racine via un nouveau callback `HomeScreen.onResults` → `Home/WarList/me` (joueur courant). Filtrage côté `WarListViewModel` (`@AssistedInject`, `@Assisted userId: String?`) : `userId` `null`/`me` = pas de filtre participation (ou joueur courant selon le contexte), sinon `WarEntity.hasPlayer(userId)`. **La war en cours (live) est exclue de l'historique dans les DEUX cas** (pôle Wars et filtré) : `.filter { it.id != currentWar?.id?.toString() }` (id entité `String` vs id war en cours `Long`) ; si aucune war en cours, la liste est inchangée. **La war en cours provient de `listenToCurrentWar(rosterId)`**, c.-à-d. la MÊME source (temps réel) que la bannière « Reprendre » (`State.currentWar`) : `state` combine réactivement `getWars()` (Room) et `listenToCurrentWar` via `flatMapLatest(rosterId) { combine(...) }`. **Cause racine du bug corrigé** : la première tentative lisait la war en cours par un `getCurrentWar(rosterId)` ponctuel dans un `zip(mkcPlayer, getWars())` — ce `zip` ne se réémettait que quand Room changeait ; or la war créée n'a PAS encore d'entrée Room (elle ne s'y écrit qu'à validation, `CurrentWarViewModel.onValidateWar` → `writeWar(WarEntity)`), donc le `zip` gardait un `currentWar` nul/périmé et n'excluait rien, alors que la bannière (listener) affichait bien la war. Partager le listener aligne exclusion et bannière. Back = `popBackStack` → retour à `StatsFullScreen` (rule 14). **Écart maquette assumé** (décision produit) : le prototype ne montre ce lien que sur `statsfull`, pas sur l'onglet Individuelles du pôle Stats.
