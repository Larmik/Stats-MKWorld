...
- `ui/MKChip.kt` : **pilule `.chip`** partagée (coins 20 dp ; actif = fond blanc/texte sombre, inactif = fond blanc translucide/texte blanc ; `enabled = false` grise le libellé et coupe le clic ; `onClick` optionnel = chip purement indicative). Extraite (rule 16) du `FilterChip` privé de `WarListScreen` dès son 2ᵉ consommateur : les **filtres de résultat** de l'historique des wars et le **compteur de lignes** `− ligne`/`N lignes`/`+ ligne` de l'écran Tab (`EditTabScreen`). Un seul exemplaire ; la version locale de `WarListScreen` a été supprimée.
...
- **Sortir les calculs lourds de la composition** : tris/filtres/sommes mémoïsés via `remember(clés)` (ex. `WarScoreView` : shocks, pénalités groupées par équipe, scores triés). Dans `AddTrackScreen` (refonte wizard, ticket #44) le `Set` des positions prises est un simple `val` en composition — le `when(step)` lit déjà largement le `state`, donc `derivedStateOf` n'y filtrerait aucune recomposition (rule 11, anti-pattern). Dans `EditTrackScreen` (refonte onglets, ticket #46), l'édition de position/shock est portée par l'état du VM (`selectedPositions`, `shocks`) ; le prédicat « toutes positions distinctes » qui active « Confirmer » (`State.positionsAllDistinct`) est calculé dans le VM, pas en composition.
...
- **Calcul de stats hors du thread UI (`withContext(Dispatchers.Default)`, #73)** : les ViewModels qui agrègent les wars (rankings pilotes/adversaires/circuits, fenêtres all-time/5/10, dashboard) faisaient leur calcul lourd sur le collecteur (`viewModelScope` = `Main.immediate`), figeant l'UI au changement de saison, au basculement de segmented et à la navigation. Correctif : la lambda de `combine`/`map` de `StatsFullViewModel`, `MapDetailViewModel`, `OpponentDetailViewModel`, `StatsRankingViewModel`, `WelcomeViewModel` et `WarListViewModel` est structurée en **deux temps** — (a) lectures de sources (Room `getSeasons`/`getWars`, DataStore, Firebase `getCurrentWar`/`listenToCurrentWar`) + filtre saison + champs légers du `State` (`seasons`, `selectedSeasonNumber`) sur le collecteur ; (b) **toute** la portion CPU lourde dans `withContext(Dispatchers.Default) { … }` — non seulement les agrégats (`withFullStats`, `computeState`/`computeRankings`, podiums), mais **aussi la construction de `WarDetails`/`War` à partir des `WarEntity`** (`map { WarDetails(War(it)) }` reparse toutes les manches/scores de chaque war, coûteux). Règle : **aucune construction de `WarDetails`/`War` ni agrégation de liste de wars ne reste hors `withContext`**. Cas explicite : le **dashboard Accueil** (`WelcomeViewModel`) construit désormais sa liste `wars` (filtre saison/roster + `map { WarDetails(War(it)) }` + tri) **dans** le `withContext`, aux côtés de `withFullStats` et de `recentResults` (un `Triple(teamStats, playerStats, recentResults)` en sort) — auparavant seul `withFullStats` y était, la construction des `WarDetails` fuyait sur le main thread → jank résiduel au changement de saison sur l'Accueil, corrigé. `StatsFullViewModel.computeState`, le bloc de `WarListViewModel` et `StatsRankingViewModel.computeRankings` incluaient déjà leur construction `WarDetails` dans le `withContext` (le `recompute()` de Classements, sur le collecteur, ne fait que filtrer/trier des listes déjà calculées → laissé tel quel). **`withContext` ciblé préféré à un `.flowOn(Dispatchers.Default)`** sur toute la chaîne (qui relocaliserait les lectures Room/Firebase/DataStore et, via `mergeWith` = `flowOf(this, flow).flattenMerge()` non ordonné, réordonnerait les émissions) — cf. **rule `.claude/rules/21-vm-offload-compute.md`**. Aucune valeur affichée ne change (rule 13). En complément, `PlayerMapsRankingScreen`/`PlayerOpponentsRankingScreen` mémoïsent le tri **et** la conversion vers `PodiumEntry` via `remember(sortIndex, source, …)` (rule 11). Cf. rules `21`, `30`, `20`, `11`.
  - **État de chargement au changement de saison (ressenti)** : le compute étant off-main mais non instantané, un `State.loading` est posé **immédiatement à `true`** par `onSeasonSelected` (via `_state`, mergé dans `state`) sur `StatsFullViewModel`, `WelcomeViewModel` et `StatsRankingViewModel` ; le compute émet ensuite `loading = false`. Le chargement est **circonscrit à la zone de données** (spinner à la place de la `LazyColumn`), les **sélecteurs de header restent visibles** : dropdown de saison (slot `headerTrailing` de `BaseScreen`, toujours hors du bloc en chargement → ne disparaît jamais), + segmenteds scope/période (`StatsFullScreen`), onglets/recherche/tri/curseur (`StatsRankingScreen`), carte de salutation + segmenté Moi/Équipe (`WelcomeScreen`). Sur Classements, `onSeasonSelected` pose `loading` via `recompute()` (conserve seasons/listes) et les interactions légères (onglet/tri/recherche/curseur) ne touchent pas `loading`. **Sur `StatsFullViewModel` (#91)** : le loader ne s'affichait pas visiblement car `_state` n'y contenait que le `State()` **par défaut** (vide) ; désormais le VM **mémorise le dernier state complet** (`private var lastComputedState`, mis à jour via `.also` sur la branche `compute`) et `onSeasonSelected` pose `lastComputedState.copy(loading = true)` — le spinner s'affiche **sans vider le header ni les données** (aligné sur `StatsRankingViewModel.recompute()`). `WarListViewModel` non concerné (calcul léger).
  - **Dédup des lectures Room invariantes (accélération réelle, iso-résultat rule 13)** : toute lecture Room/DataStore **invariante** (indépendante de la fenêtre/scope) doit être lue **une fois** en amont et passée en paramètre, jamais répétée dans une boucle par fenêtre/vue. Appliqué dans `StatsFullViewModel.computeState` : la liste des équipes adverses (`getTeams().firstOrNull().filterNot { id == currentTeamId }`) était relue **6×** (2 vues × 3 fenêtres) par `computeOpponentRankings` → hoistée une fois (`opponentTeams`, param), **6 lectures → 1** ; le tri chronologique des wars des contributeurs (refait 3× par fenêtre) réutilise `chronologicalWars` déjà trié. `computeContributorsByWindow` lisait déjà `mkcTeam`/`getPlayers` une seule fois. **Point chaud résiduel** (non traité, ticket de suivi, à profiler device) : `withFullStats` reste appelé **N membres × 3 fenêtres** dans les contributeurs, **2 × 3** pour player/team stats, et `withFullTeamStats` **6×** (2 vues × 3 fenêtres) — calcul paresseux « seulement la fenêtre/vue visible » possible mais structurel.
...
- **Création de war** : l'hôte est enregistré via son `rosterId` (`PlayerEntity.rosterId` mkworld) et, depuis la sélection de roster adverse (`AddWarViewModel`), l'adversaire aussi. À la sélection d'un adversaire, `AddWarViewModel.onTeamSelected()` appelle `mkCentralDataSource.getTeam(teamId)` et filtre `game == "mkworld"` : **un seul roster** → le `rosterId` est retenu directement (`commitTeam`, passage à l'étape 2) ; **plusieurs rosters** → un **sélecteur inline** se déplie sous la ligne (`State.expandedRosterTeamId`), et `onRosterSelected(team, roster)` retient le roster choisi. `commitTeam` applique une **garde d'unicité** (pas de double ajout du même adversaire, #91). Le `rosterId` par adversaire n'est plus mémorisé dans une `var` parallèle (`selectedRosterIds` supprimée, #91) : il est **dérivé** de `State.rostersSelected`/`teamSelected` (rosterId mkworld, sinon fallback teamId ; index aligné sur `teamSelected`) puis écrit dans `War.teamOpponent` par `createWar()`.
...
**Contraste victoire/défaite (#50).** `Colors.red`/`Colors.green` (défaite/victoire) ont été **assombris/saturés** par rapport aux pastels de la maquette (`--loss #F28B82` → `#E05D51`, `--win #81C995` → `#4FA96C`) : ces couleurs servent de **texte** sur le fond clair du dégradé de `BaseScreen` où les pastels d'origine manquaient de contraste, et elles restent lisibles sur carte sombre comme en texte blanc sur pastille V/D. Le **dégradé du CTA** (`MKButton` Gradient) conserve, lui, les pastels d'origine (stops figés dans `MKButton`, indépendants de `Colors`). Ajout de `Colors.appbar` (rgba(48,51,54,.5)) pour la bande d'appbar (#50 pt.2).
...
| `scoreStdDev` | écart-type (population) des scores par war ; null si < 2 wars | `chronologicalScores.score` | Régularité (fenêtre via `FormStats`) |
| `scoreMin` / `scoreMax` | amplitude min/max des scores par war | idem | Amplitude (fenêtre via `FormStats`) |
| `positionDistribution` | **vue joueur** : `List<Pair<pos, count>>` sur **1..12 en 12p, 1..24 en 24p** (étendue mode-aware via `Stats.is24p`, ne tronque plus les positions 13..24 en 24p) depuis les `WarPosition` du joueur | `chronologicalWars.tracks` | `MKDistributionCard` (histogramme, vue joueur — rendu couleurs P1→P24 : ticket UI dédié) |
| `penaltyPointsLost` | Σ des `WarPenalty` de l'équipe hôte sur l'historique | `War.penalties` | Section Records & séries (fenêtre via `FormStats`) |
...
Le résultat est affiché par `ui/cells/WarPlayerRankingCard` (grille 2 colonnes de tuiles nom + points, triées par points décroissants) sur l'écran **Détail de war** (`WarDetailsScreen`, refonte #48). Sur **War en cours** (`CurrentWarScreen`, refonte #43), la liste des `PlayerScore` est rendue par le composable local `PlayersCard` (tuiles nom + points en deux colonnes, style carte dashboard de la maquette) — dispositions volontairement distinctes dans la maquette. `CurrentWarScreen` étant poussé sur le **graphe racine** (par-dessus le pôle, **sans** bottombar), la marge basse (~90 dp) réservée à la bottombar a été **retirée** (superflue, #67 ; rule 17 ne concerne que les écrans de pôle).

**Composants de résumé de war mutualisés `ui/cells/WarSummaryCells.kt`** (refonte #48, rule 16) : `WarDetailsScreen` (war terminée) est l'**écran-frère** de `CurrentWarScreen` (war en cours) dans le pôle Wars de la maquette 5 pôles. Les composants de résumé partagés entre les deux ont été extraits dans ce fichier et rendus publics : `WarScoreCard` (carte « Score du match » — hôte VS adversaire(s), pastille avatar/initiales, nom de roster, score, **différence signée colorisée au centre** via `Int.diffColor`, pénalités par équipe et total de shocks ; `subtitle` optionnel = « N courses restantes » côté war en cours, absent côté war terminée), `WarTracksSection` (carte englobante « Courses jouées · N » + grille 2 colonnes de `MKTrackCell`, chaque cellule → détail de course via un numéro 1-based), plus les briques `WarDashboardCard`/`WarEyebrow`/`WarTeamSide`/`WarTeamCrest` (style carte dashboard `blackAlphaed` + bordure blanche, radius 6, pixel-perfect maquette rules 13/15) et `WarPlayerRankingCard` (spécifique à la war terminée). `CurrentWarScreen` consomme les mêmes `WarScoreCard`/`WarTracksSection`, éliminant la duplication.
...
> **Dead code — `ui/WarScoreView.kt`.** L'ancien composant monolithique `WarScoreView` (scores finaux 12/24, tableau des 3 équipes en 24p, sections pénalités/shocks — cf. audit D20) n'est **plus référencé** depuis la refonte #48 : `WarDetailsScreen` utilise désormais `WarScoreCard` (`WarSummaryCells.kt`) et `CurrentWarScreen` son propre `ScoreCard`. Le fichier subsiste (code mort à supprimer par un ticket de nettoyage dédié) ; les mentions de `WarScoreView` ailleurs dans ce document décrivent l'état antérieur.
...
>
> **Le MÊME modèle `SeasonFilter` (`Default`/`AllTime`/`Specific`) + `_seasonFilter` combiné au flux de données est appliqué par 4 ViewModels** : `WelcomeViewModel` (agrégats du dashboard : momentum/séries/records/chiffres clés/derniers résultats — `filterBySeason` avant `withFullStats`), `WarListViewModel` (filtre la liste affichée), `StatsFullViewModel` et `StatsRankingViewModel`. **Momentum & saison** : le momentum de l'Accueil se déduit des wars **déjà filtrées par saison** passées à `withFullStats` → en vue saison, `chronologicalOutcomes`/`scoreTimeline` (et leurs `takeLast(5/10)`) portent sur cette saison ; en « Tout l'historique », sur toute l'histoire — aucune logique dédiée.
>
> **Sélecteur UI — dropdown partagé (retour utilisateur)** : le sélecteur de saison est un **menu déroulant `ui/MKSeasonDropdown.kt`** — composant **stateless partagé unique** (rule 16) réutilisé par les headers **Accueil, Wars, Stats, Classements**. Il est rendu via le slot **`headerTrailing`** de `BaseScreen` (aligné à droite dans l'app bar, avant l'icône d'action) → il reste **affiché en permanence**, indépendamment du corps scrollable et de la vacuité de la liste filtrée. Paramètres : `seasons`, `selectedSeasonNumber` (null = tout l'historique), `onSeasonSelected(Int?)` ; seul l'état d'ouverture du menu est local (`mutableStateOf`). Trigger + `DropdownMenu` sont dans un `Box(wrapContentSize(Alignment.TopEnd))` → le popup **s'ancre à droite** (bord droit aligné sur le sélecteur), ne débordant plus à gauche. L'item **sélectionné** est mis en évidence en **`Colors.green`** (accent existant, lisible sur le fond clair du menu — remplace le jaune peu contrasté). L'ancien `MKSegmentedSelector` de saison (Stats/Classements) et la pastille « Saison N » (Accueil) ont été **supprimés** (pas de doublon résiduel). ⚠️ **Écart maquette assumé (rules 13/15)** : le prototype ne prévoit pas de dropdown de saison — style aligné sur les boutons d'app bar (fond `white30`, bordure douce, chevron texte « ▾ » faute d'asset de flèche bas).
>
> **Observation réactive des saisons (#73)** : les 4 VM headers (`WelcomeViewModel`, `WarListViewModel`, `StatsFullViewModel`, `StatsRankingViewModel`) lisent désormais la liste des saisons via une **source `combine` réactive** `databaseRepository.getSeasons()` (Flow Room streaming — rule 30) **au lieu** d'un `getSeasons().firstOrNull()` one-shot dans la lambda. Ainsi, quand l'hydratation eager (InitStatsWorker / SignupViewModel) écrit les saisons, le Flow ré-émet → le `combine` re-run → le dropdown apparaît **sans redémarrage**. La liste `seasons` reste résolue **sur le collecteur, hors `withContext(Default)`** (rule 21 : c'est un champ léger de `State` qui doit rester peuplé même si le calcul de stats est nul/vide). Les VM détail (Map/Opponent) n'ont pas de dropdown → pas concernés.
>
> **Persistance du dropdown sur Classements (retour utilisateur)** : les interactions du pôle Classements (onglet/tri/recherche/curseur) écrivent un `_state` partiel consommé via `mergeWith` ; ce `_state` interne n'étant PAS alimenté par la branche `combine` (qui charge les saisons), les `seasons` y étaient vides → le dropdown (masqué si `seasons` vide) disparaissait après toute interaction. Corrigé en **mémorisant** `loadedSeasons`/`loadedSelectedSeasonNumber` (champs privés du VM, comme `allMembers`…) et en les **ré-injectant à chaque `recompute()`**. Le filtre saison (`filterBySeason`) s'applique à `warList`, base des **quatre** classements (membres, alliés, adversaires, circuits).
>
> **Pas de libellé de saison sur les records** (retour utilisateur) : la feature d'affichage « SN » sur les tuiles de « Records & séries » a été **retirée**. Le code mort associé a été supprimé — champs `Stats.scoreMinWarId`/`scoreMaxWarId`/`bestWinStreakWarId`/`worstLossStreakWarId` + helper `longestStreakEndWarId`, `MetricTile.caption`, `seasonLabelForWar`, et le paramètre `seasons` de `RecordsTilesCard` (string `season_short` supprimée). Seul le **filtrage** par saison des agrégats subsiste ; aucun libellé de saison n'est plus rendu sur records/séries.
...
- **Podiums (tri 3 dimensions, Occurrences par défaut)** : « Meilleurs / pires circuits » et « adversaires » en **Top 3 / Flop 3, chacun sur UNE ligne** (`PodiumRow`). Sélecteur `Occurrences | Winrate | Score` (défaut Occurrences) :
  ...
  - **Dégradation propre par période (#91)** : sur une **fenêtre réduite** (5/10 dernières), le seuil `MIN_RANKING_SAMPLE` peut vider le tri Winrate/Score alors que les Occurrences restent peuplées → auparavant la carte adversaires/circuits **disparaissait** (ou affichait un podium tronqué « 2 sur 3 »). Correctif **purement affichage** (le seuil VM est **inchangé**, rule 13) dans `StatsFullScreen` : les cartes ne se masquent que si **aucune** entrée n'est classable **tous tris confondus** ; le helper `PodiumOrMessage(label, entries)` ne rend un `PodiumRow` **que si le podium est complet (3 entrées)**, sinon un **message** (`stats_podium_not_enough`) — jamais de podium partiel incohérent.
...
### SeasonRepository
Repository **dédié** à la notion de **saison** (#30). Agrège deux sources sans repository naturel unique — RTDB (`FirebaseRepository`, source de vérité `seasons/{teamId}`) et Room (`DatabaseRepository`, cache local `SeasonEntity`) — d'où un repository propre (interface + module `@Binds @Singleton`) plutôt qu'une extension d'un UseCase partagé (rule 32). Trois responsabilités :
...
**Écran `EditTabScreen` — refonte pixel-perfect (maquette `edittab`, #49, rules 13/15/16).** Titre `Tab (PDF)`. `LazyColumn` : **chips compteur** (composant partagé `MKChip` : `− ligne` / `N lignes` active / `+ ligne`, chips `±` grisées en butée min 6 / max 9) → lignes de saisie `Adversaire N` (large) + `Score` (étroit) en grille 2/1 (`MKTextField`, `key = index`, rule 10) → **CTA** `Tab classique & partager` (**`MKButton` unique**, param `icon = ic_share`, `text = R.string.tab_share_cta` — l'ancien `TabShareCta` gradient ad hoc a été **aplati sur `MKButton`**, #67, rule 16 ; plus aucun fond de bouton coloré). **Sans hints** (les textes d'aide d'intro/contenu ont été retirés à la demande de l'utilisateur). L'état de saisie tient dans deux `mutableStateListOf` de **9 emplacements** (max) ; seules les `rows` (`StateFlow` du VM, `onManageRows`) premières lignes sont affichées et transmises à `generateClassicPdf` (`take(rows)`), réduire le compteur ne détruit pas la saisie. `EditTabViewModel.generateClassicPdf` **inchangé** (justesse scores/pénalités prioritaire, rule 13) : valide `Σ scores adverses == scoreOpponent`, résout hôte/adversaires (rosterId → équipe parente pour nom/tag/logo, rule 12), détermine gagnant/perdant via `scoreHostWithPenalties >= scoreOpponentWithPenalties` (**pénalités incluses**), génère puis partage. `generateDetailedPdf` (Tab détaillé : circuits + courbe) reste **commenté** = présent mais désactivé.
