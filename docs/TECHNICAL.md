...
>
> **Libellés de saison sur les records** : `Stats` expose désormais `scoreMinWarId`/`scoreMaxWarId`/`bestWinStreakWarId`/`worstLossStreakWarId` (war.id — timestamp — du record all-time). En vue **« Tout l'historique » ET période all-time** uniquement, `RecordsTilesCard` résout la saison contenant chaque war.id (`seasonLabelForWar`) et affiche « SN » en légende de tuile ; ces champs sont additifs (aucun calcul de stats existant modifié).
