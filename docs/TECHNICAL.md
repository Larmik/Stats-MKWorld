...
- **Sections détaillées mutualisées** (`ui/stats/MapStatsSections.kt`, `LazyListScope.mapStatsDetailSections(MapStats)`) : ajoutées **aux deux fiches** — même calcul/rendu que `StatsFullScreen`, scopé à l'entité. Répartition des positions (`ui/stats/MKDistributionCard.kt` : `DistributionChart`/`DistributionFooter` **extraits** de `StatsFullScreen`, rule 16 — `StatsFullScreen` les importe désormais), Top/Bot 2→6 (`ui/stats/MKTopBottomStatsCard.kt` : `TopBottomColumns`, style maquette dans une `StatCard`), et shocks. Alimentées par `MapStats.positionDistribution` (nouvelle propriété : positions de l'ÉQUIPE hôte, ou du joueur en vue indiv, P1→P12/P1→P24), `topsTable`/`bottomsTable`, `shockCount`.
