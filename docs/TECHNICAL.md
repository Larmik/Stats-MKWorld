...
- **Sélecteurs de section** — état UI hissé dans `StatsFullScreen` (`rememberSaveable`, rule 11) et passé aux sections via `SectionSelectors` (les sections sont des extensions `LazyListScope`) :
  - **Fenêtre Indicateurs** (all-time / 5 / 10) : la carte lit `Stats.windowForm(index)` (`allTimeForm`/`recentForm5`/`recentForm10`) et affiche valeur + **delta % vs all-time** (flèche ↗/↘ colorée, uniquement sur le delta ; valeurs en blanc). `FormStats` a été **étendu** pour porter par fenêtre `scoreStdDev`, `scoreMin`/`scoreMax`, `winMargin`/`lossMargin`, **`bestWinStreak`/`worstLossStreak`/`top6Count`/`bot6Count`** en plus de winrate/score/position/mapsWon/shocks. En vue **Équipe**, la tuile « Score moyen » affiche l'**écart** (`averageScore.warScoreToDiff(false)`), pas le total ; en **Individuelles**, points/war bruts.
  - **Fenêtre Records** (all-time / 5 / 10) : **sélecteur indépendant** (`recordsWindowIndex`) recalculant la section sur `windowForm` (amplitude min/max, records V/D, Top6/Bot6 par fenêtre).
  - **Tri podiums** winrate / score : bascule `topMapsByWinrate`↔`topMapsByScore` (+ flop) et les classements adversaires winrate↔score.
- **Grilles régulières + hauteur figée** : `MetricTiles` (Indicateurs & Records) dispose `columns` colonnes à poids égal ; **chaque tuile réserve la ligne de progression** (`Box(Modifier.height(DeltaSlotHeight))`, occupée même sans delta) → toutes les tuiles ont exactement la même hauteur, calée sur la plus grande (celle avec delta), et **aucune tuile ne se redimensionne** au changement de fenêtre (plus de « saut » de layout).
- **Podiums** : « Meilleurs / pires circuits » et « Meilleurs / pires adversaires » en **Top 3 / Flop 3, chacun sur UNE ligne** (`PodiumRow` : jusqu'à 3 cellules à poids égal, `IntrinsicSize.Min`). Une **cellule podium dédiée** (`PodiumCell`, alimentée par `PodiumEntry`) est **partagée** entre circuits et adversaires : **structure identique** (image en haut, nom 2 lignes, valeur en gras), seule l'image change — illustration de map (`Maps.picture`, arrondie) vs logo d'équipe (`AsyncImage` MKCentral / `default_logo`, cercle). Valeur = winrate % (tri winrate) ou score (tri score : score joueur si `userId`, sinon score/écart d'équipe). Les `MapCell`/`TeamCell` historiques (trop larges pour 3-par-ligne) ne sont plus utilisées ici.
...
- **Pénalités** : dans les Indicateurs, la tuile Amplitude a été remplacée par « Points perdus en pénalités » (`Stats.penaltyPointsLost`, cumul all-time) ; l'amplitude est passée dans Records.
- **Distribution des positions** : barres dans une **zone à hauteur fixe** ancrées `BottomCenter` (ligne de base commune) → labels de position **alignés horizontalement**.
